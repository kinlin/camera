...
from colour_demosaicing import demosaicing_CFA_Bayer_Malvar2004 as demosaicingA
from colour_demosaicing import demosaicing_CFA_Bayer_Menon2007 as demosaicingB
from colour_demosaicing import demosaicing_CFA_Bayer_DDFAPD as demosaicingC
import matplotlib.pyplot as plt

g_rgbGain = np.array([1.7, 1, 1.9])
...
def raw16torawf(raw, h):
    return raw.reshape((h, -1))/np.float(2**10)
...
class RawBayerImage(RawImageBase):
    ...
    def load(self):
        ...
        rawf = rawfAwb(rawf, g_rgbGain[0], g_rgbGain[2], self.bayer)
        self.rgb = demosaicingB(rawf, self.bayer)
